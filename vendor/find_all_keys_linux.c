/*
 * find_all_keys_linux.c - Linux Telegram memory key scanner
 *
 * Scans Telegram process memory for SQLCipher encryption keys in the
 * x'<key_hex><salt_hex>' format.
 *
 * Usage:
 *   sudo ./find_all_keys_linux [pid] <db_storage_path>
...
int tg_scan_keys_linux(int argc, const char *argv[]) {
    pid_t pid = -1;
    ...
    printf("============================================================\n");
    printf("  Linux Telegram Memory Key Scanner\n");
    printf("============================================================\n");
    ...
    /* Open /proc/<pid>/mem and /proc/<pid>/maps */
    char mem_path[64];
    char maps_path[64];
    ...
    if (!mem_fp) {
        fprintf(stderr, "Cannot open %s: %s (run as root)\n", mem_path, strerror(errno));
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), maps_fp)) {
        ...
        /* Only scan readable+writable private regions */
        if (perms[0] != 'r' || perms[1] != 'w' || perms[3] != 'p')
            continue;

        region_count++;
        unsigned long ca = start;
        while (ca < end) {
            size_t cs = end - ca;
            ...
            ssize_t n = pread(fileno(mem_fp), buf, cs, (off_t)ca);
            if (n > 0) {
                total_scanned += n;

                for (size_t i = 0; i + HEX_PATTERN_LEN + 3 < (size_t)n; i++) {
                    if (buf[i] == 'x' && buf[i + 1] == '\'') {
                        int valid = 1;
...
int main(int argc, char *argv[]) {
    return tg_scan_keys_linux(argc, (const char **)argv);
}
#endif
